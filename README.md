...
These steps take total boot time from ~30s down to ~17s (systemd portion) and eliminate visible boot text/login prompts on the display:
...
**Disable the Pi 5 bootloader diagnostics/QR code screen** (shown on cold boot only):
```bash
